# Traefik Helm Chart Blueprint

## General

The blueprint installs Traefik on top of Kubernetes cluster using the official Bitnami Helm chart.
...
Example:
```shell
cfy install blueprint.yaml -b helm-traefik -d helm-traefik -l csys-obj-parent=K8S_DEPLOYMENT_ID
```
...
### Release
the node type is responisble for installing the Traefik deployment using the official Bitnami chart.
...
| Name             | Description                           |
| ---------------- | ------------------------------------- |
| traefik_endpoint | URL of the Traefik ingress            |
