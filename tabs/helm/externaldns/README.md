# External-dns Helm Chart Blueprint

## General

The blueprint installs External-dns on top of Kubernetes cluster using the official Bitnami Helm chart.
...
Example:
```shell
cfy install blueprint.yaml -b helm-External-dns -d helm-External-dns -l csys-obj-parent=K8S_DEPLOYMENT_ID
```
...
| -------------------------------------- | ------------- | ------ | ------------------- |
| The port number used for broker access | access_port   | string | "7979"              |
| Name of the helm release               | release_name  | string | exampleExternal-dns |
...
### Release
the node type is responisble for installing the External-dns deployment using the official Bitnami chart.
...
| Name                      | Description                     |
| ------------------------- | ------------------------------- |
| External-dns_access_point | URL of the External-dns access  |
