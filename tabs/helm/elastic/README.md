# Elastic Helm Chart Blueprint

## General

The blueprint installs Elastic on top of Kubernetes cluster using the official Bitnami Helm chart.
...
Example:
```shell
cfy install blueprint.yaml -b helm-elastic -d helm-elastic -l csys-obj-parent=K8S_DEPLOYMENT_ID
```
...
| Kubernetes replica count for the StatefulSet (i.e. how many pods) | replicas      | string | '1'              |
...
### Release
the node type is responisble for installing the Elastic deployment using the official Bitnami chart.
...
| Name                     | Description  |
| ------------------------ | ------------ |
| elastic load balancer ip | elastic ip   |
