# Mysql Helm Chart Blueprint

## General

The blueprint installs Mysql on top of Kubernetes cluster using the official Bitnami Helm chart.
...
Example:
```shell
cfy install blueprint.yaml -b helm-mysql -d helm-mysql -l csys-obj-parent=K8S_DEPLOYMENT_ID
```
...
### Release
the node type is responisble for installing the Mysql deployment using the official Bitnami chart.
...
| Name          | Description                  |
| ------------- | ---------------------------- |
| root_password | Passsword for the root user  |
| server_ip     | Mysql server ip address      |