# Postgres Helm Chart Blueprint

## General

The blueprint installs Postgres on top of Kubernetes cluster using the official Bitnami Helm chart.
...
Example:
```shell
cfy install blueprint.yaml -b helm-postgres -d helm-postgres -l csys-obj-parent=K8S_DEPLOYMENT_ID
```
...
| Name                | Description                          |
| ------------------ | ------------------------------------- |
| postgres_server_ip | Endpoint for the database connection  |
| postgres_admin_pwd | 'postgres' user password              |