# Prometheus Helm Chart Blueprint

## General

The blueprint installs Prometheus on top of Kubernetes cluster using the official Bitnami Helm chart.
...
Example:
```shell
cfy install blueprint.yaml -b helm-prometheus -d helm-prometheus -l csys-obj-parent=K8S_DEPLOYMENT_ID
```
...
### Release
the node type is responisble for installing the Prometheus deployment using the official Bitnami chart.
...
| Name          | Description                           |
| ------------- | ------------------------------------- |
| main_page     | Main page of the Prometheus console   |